...
namespace MLOps.NET.IntegrationTests
{
    public class DeploymentCatalogTests : RepositoryTests
    {
        ...
        public async Task BuildAndPushImageAsync_ShouldAddPackageDependencies()
        {
            ...
            references.Any(x => x.Contains(@"Include=""Microsoft.ML"" Version=""1.6.0""")).Should().BeTrue();
            references.Any(x => x.Contains(@"Include=""Microsoft.ML.CpuMath"" Version=""1.6.0""")).Should().BeTrue();
            references.Any(x => x.Contains(@"Include=""Microsoft.ML.DataView"" Version=""1.6.0""")).Should().BeTrue();
        }
    }
}
